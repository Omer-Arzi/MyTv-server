...
- **One Piece** — `TITLE_MISMATCH` in the TV Time parity audit. Two MyTv rows exist ("One Piece," ~1,157 episodes watched, and "ONE PIECE (2023)," 16 episodes) and is also a duplicate-title-group member (`image-coverage-audit`). The only cached TMDb candidate has just 17 known episodes — clearly belongs to the 2023 live-action entry, not the long-running anime. **Resolved 2026-08-16 for the anime row only** (`ONE PIECE (2023)` stays listed): that TITLE_MISMATCH risk is about a title-search candidate lookup landing on the wrong entry, which never applied here — this row was already correctly matched to TMDb id 37854 (confirmed by cross-checking the user's real last-watched episode, 1169, against TMDb's live catalog: exact title + air-date match) and only ever refreshes by that known id, never by re-searching the title. Removed from `EPISODE_NUMBERING_RISK_LIST_TITLES`.
